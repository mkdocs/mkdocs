...
log = logging.getLogger('elstir.config')
...
class Config(UserDict):
    ...
    def load_file(self, config_file):
        try:
            ...
            # elstir knows and understands ConfigurationErrors
            raise exceptions.ConfigurationError(
                "elstir encountered as error parsing the configuration file: {}".format(e)
            )
...
def _open_config_file(config_file):
    ...
    if config_file is None:
        config_file = os.path.abspath('elstir.yml')
...
def load_config(config_file=None, **kwargs):
    ...
    the default `elstir.yml` filename will loaded.
    ...
    from elstir.config.defaults import get_schema
