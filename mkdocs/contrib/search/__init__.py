    ...
    def run_validation(self, value):
        ...
        for lang in value:
            if lang != "en" and not self.lang_file_exists(lang):
                raise config_options.ValidationError(f'"{lang}" is not a supported language code.')
        return value
...
class SearchPlugin(BasePlugin):
    ...
    config_scheme = (
        ...
        (
            "prebuild_index",
            config_options.Choice((False, True, "node", "python"), default=False),
        ),
        (
            "indexing",
            config_options.Choice(("full", "sections", "titles"), default="full"),
        ),
    )
