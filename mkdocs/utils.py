...
class RelativePathTreeprocessor(Treeprocessor):
    ...
    def run(self, root):
        ...
        for element in RelativePathTreeprocessor._iter(root):
            ...
            url = element.get(key)
            new_url = RelativePathTreeprocessor.path_to_url(url, self.site_navigation, self.strict)
    ...
    def path_to_url(url, nav, strict):
        ...
        if nav and not is_markdown_file(path):
            path = create_relative_media_url(nav, path)
        elif nav:
            ...
            if target_file not in nav.source_files:
                ...
                log.warning(msg)
                return url
            path = get_url_path(target_file, nav.use_directory_urls)
            path = nav.url_context.make_relative(path)
        else:
            path = get_url_path(path).lstrip('/')
