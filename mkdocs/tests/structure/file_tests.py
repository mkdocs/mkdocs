...
class TestFiles(PathAssertionMixin, unittest.TestCase):
    ...
    def test_md_file(self):
        ...
        self.assertFalse(f.is_link_file())
    ...
    def test_link_file(self):
        f = File('foo/bar.url', '/path/to/docs', '/path/to/site', use_directory_urls=False)
        self.assertEqual(f.src_uri, 'foo/bar.url')
        self.assertPathsEqual(f.abs_src_path, '/path/to/docs/foo/bar.url')
        self.assertEqual(f.dest_uri, 'foo/bar.url')
        self.assertPathsEqual(f.abs_dest_path, '/path/to/site/foo/bar.url')
        self.assertEqual(f.url, 'foo/bar.url')
        self.assertEqual(f.name, 'bar')
        self.assertFalse(f.is_documentation_page())
        self.assertFalse(f.is_static_page())
        self.assertTrue(f.is_link_file())
        self.assertFalse(f.is_media_file())
        self.assertFalse(f.is_javascript())
        self.assertFalse(f.is_css())

    def test_link_file_use_directory_urls(self):
        f = File('foo/bar.url', '/path/to/docs', '/path/to/site', use_directory_urls=True)
        self.assertEqual(f.src_uri, 'foo/bar.url')
        self.assertPathsEqual(f.abs_src_path, '/path/to/docs/foo/bar.url')
        self.assertEqual(f.dest_uri, 'foo/bar.url')
        self.assertPathsEqual(f.abs_dest_path, '/path/to/site/foo/bar.url')
